...
const C4K_I18N = {
	en: {
		...
		'card.admin_title': 'Udfordringer – Parents',
		...
		// Child specific
		'card.child_title_fallback': 'Udfordringer – {name}',
		...
		'msg.no_tasks': 'No tasks right now.',
			...
			'editor.completion_sound_help':'Saved as /local/udfordringer/completion.<ext> and used across dashboards',
	...
	da: {
		...
		'card.admin_title': 'Udfordringer – Forældre',
		...
		'shop.title':'Pointshop','shop.open':'Åbn shop','shop.item':'Vare','shop.price':'Pris','shop.icon':'Ikon','shop.image':'Billede','shop.upload':'Upload billede','shop.add_item':'Tilføj vare','shop.history':'Købshistorik','shop.child':'Barn','shop.when':'Tidspunkt','shop.date':'Dato','shop.time':'Tidspunkt','shop.advanced':'Avancerede handlinger','shop.entity':'Enhed','shop.operation':'Handling','shop.add_action':'Tilføj handling','shop.add_delay':'Tilføj delay','shop.steps':'Trin','shop.seconds':'Sekunder','shop.minutes':'Minutter','shop.hours':'Timer','shop.delay':'Forsinkelse','shop.active':'Aktiv','shop.buy':'Køb','shop.bought':'Købt','shop.clear_history':'Ryd historik','confirm.clear_history':'Ryd hele købshistorikken?',
			...
			'editor.completion_sound':'Completion Sound',
			'editor.completion_sound_help':'Gemmes som /local/udfordringer/completion.<ext> og bruges på tværs af dashboards',
			...
			'card.admin_title': 'Udfordringer – Förälder', 'input.new_child_name': 'Nytt barn — namn', 'btn.add_child': 'Lägg till barn',
			...
			'editor.completion_sound_help':'Sparas som /local/udfordringer/completion.<ext> och används på alla dashboards',
			...
			'card.admin_title':'Udfordringer – Forelder','input.new_child_name':'Nytt barn — navn','btn.add_child':'Legg til barn',
			...
			'editor.completion_sound_help':'Lagres som /local/udfordringer/completion.<ext> og brukes på tvers av dashboards',
			...
			'card.admin_title':'Udfordringer – Eltern','input.new_child_name':'Neues Kind — Name','btn.add_child':'Kind hinzufügen',
			...
			'editor.completion_sound_help':'Wird als /local/udfordringer/completion.<ext> gespeichert und auf allen Dashboards verwendet',
			...
			'card.admin_title':'Udfordringer – Padres','input.new_child_name':'Nuevo niño — nombre','btn.add_child':'Añadir niño',
			...
			'card.child_title_fallback':'Udfordringer – {name}','msg.child_not_found':'Niño no encontrado. Revisa el nombre en la configuración de la tarjeta.','msg.no_tasks':'No hay tareas por ahora.', 'btn.done':'Hecho','btn.start_task':'Iniciar tarea','btn.complete_task':'Tarea hecha','btn.task_done':'Tarea hecha','btn.complete_bonus':'Completar bono','lbl.awaiting':'En espera de aprobación','lbl.bonus':'Tarea de bono','lbl.bonus_locked':'Completa la tarea principal primero','lbl.taken_by':'Tarea tomada por {name}','lbl.points':'puntos',
			...
			'editor.completion_sound_help':'Se guarda como /local/udfordringer/completion.<ext> y se usa en todos los paneles',
			...
			'card.admin_title':'Udfordringer – Parents','input.new_child_name':'Nouvel enfant — nom','btn.add_child':'Ajouter un enfant',
			...
			'editor.completion_sound_help':'Enregistré sous /local/udfordringer/completion.<ext> et utilisé sur tous les tableaux de bord',
			...
			'card.admin_title':'Udfordringer – Vanhemmat','input.new_child_name':'Uusi lapsi — nimi','btn.add_child':'Lisää lapsi',
			...
			'editor.completion_sound_help':'Tallennetaan muotoon /local/udfordringer/completion.<ext> ja käytetään kaikilla näkymillä',
			...
			'card.admin_title':'Udfordringer – Genitori','input.new_child_name':'Nuovo bambino — nome','btn.add_child':'Aggiungi bambino',
			...
			'editor.completion_sound_help':'Salvato come /local/udfordringer/completion.<ext> e usato su tutte le dashboard',
...
class UdfordringerDevCard extends LitElement {
	...
	constructor(){
		...
		try{ this._iconRecents = JSON.parse(localStorage.getItem('udf_icn_recent')||'[]') || []; }catch{ this._iconRecents = []; }
		try{ this._customIcons = JSON.parse(localStorage.getItem('udf_custom_icons')||'[]') || []; }catch{ this._customIcons = []; }
		this._iconSearch = '';
		this._availableIcons = [];
		// sorting
		try{ this._catOrder = JSON.parse(localStorage.getItem('udf_cat_order')||'[]') || []; }catch{ this._catOrder = []; }
		// collapsed sections
		try{ this._collapsed = JSON.parse(localStorage.getItem('udf_admin_collapsed')||'{}') || {}; }catch{ this._collapsed = {}; }
		// completed timestamps
		try{ this._completed = JSON.parse(localStorage.getItem('udf_completed_ts')||'{}') || {}; }catch{ this._completed = {}; }
		// child-only visual bonus progress (no backend persistence)
		try{ this._bonusVisual = JSON.parse(localStorage.getItem('udf_bonus_visual')||'{}') || {}; }catch{ this._bonusVisual = {}; }
	...
	connectedCallback() {
		...
		this._storageInterval = setInterval(() => {
			try {
				const stored = JSON.parse(localStorage.getItem('udf_completed_ts') || '{}');
				...
				const bonusStored = JSON.parse(localStorage.getItem('udf_bonus_visual') || '{}');
	...
	_handleStorageChange(e) {
		if (e.key === 'udf_completed_ts' && e.newValue) {
			try {
	...
	_getBackendUiColors(){
		try{
			...
			let s = states['sensor.udfordringer_ui'];
			if (!s){
	...
	static getConfigElement(){ return document.createElement('udfordringer-card-editor'); }
	...
	_recordCompleted(taskId, ts){ try{ const map={...(this._completed||{})}; map[String(taskId)]=ts; this._completed=map; localStorage.setItem('udf_completed_ts', JSON.stringify(map)); }catch{} }
	_clearCompleted(taskId){ try{ const map={...(this._completed||{})}; delete map[String(taskId)]; this._completed=map; localStorage.setItem('udf_completed_ts', JSON.stringify(map)); }catch{} }
	...
	get _store(){
		...
		if (!allTasksSensor){ allTasksSensor = Object.values(states).find((s)=> s?.entity_id?.includes('udfordringer_tasks') && s.attributes?.tasks); if (allTasksSensor?.entity_id) this._idTasks = allTasksSensor.entity_id; }
		...
		if (!shopSensor){ shopSensor = Object.values(states).find((s)=> s?.entity_id?.includes('udfordringer_shop')); if (shopSensor?.entity_id) this._idShop = shopSensor.entity_id; }
	...
	_toggleSection(key){ const next={ ...(this._collapsed||{}) }; next[key]=!this._isCollapsed(key); this._collapsed=next; try{ localStorage.setItem('udf_admin_collapsed', JSON.stringify(next)); }catch{} this.requestUpdate(); }
	...
	_setBonusVisualState(taskId, state){
		...
		try{ localStorage.setItem('udf_bonus_visual', JSON.stringify(next)); }catch{}
		this.requestUpdate();
	}
	...
	async _onChildBonusVisualClick(t){
		...
		try{
			if (!t?.bonus_completed_ts){
				await this.hass.callService('udfordringer','complete_bonus_task',{ task_id: t.id, completed_ts: Date.now() });
	...
	async _approveBonusOnly(t){
		...
		try{
			...
			await this.hass.callService('udfordringer','approve_bonus_task',{ task_id: t.id });
			this._setBonusVisualState(t.id, 'done');
		} finally {
			this._setTaskBusy(t.id, false);
		}
	...
	_renderSortModal(){
		...
		const save = ()=>{ try{ localStorage.setItem('udf_cat_order', JSON.stringify(order)); }catch{} this._catOrder = order; this._sortModalOpen=false; this.requestUpdate(); };
	...
	_renderReassignModal(){
		...
		const approveOnly = async () => {
			await this.hass.callService('udfordringer','approve_task',{ task_id: task.id });
			...
			for (const childId of targetIds) {
				try {
					...
					if (templates.length > 0) {
						await this.hass.callService('udfordringer','assign_task',{ 
	...
	_renderChild(){
		...
		if (!shopSensor){ shopSensor = Object.values(this.hass.states).find(st=> st?.entity_id?.includes('udfordringer_shop')); if (shopSensor?.entity_id) this._idShop = shopSensor.entity_id; }
	...
	// ===== Actions (admin + child) =====
	async _addChild(){ if(!this._name) return; await this.hass.callService('udfordringer','add_child',{ name: this._name }); this._name=''; }
	async _removeChild(c){ if(!confirm(this._t('confirm.delete_child',{name:c.name}))) return; await this.hass.callService('udfordringer','remove_child',{ child_id: c.id }); }
	...
	async _addPointsQuick(n){ if(!this._pointsEnabled()) return; if(!this._pointsChild) return; const pts=Number(n||0); if(!Number.isFinite(pts)||pts===0) return; await this.hass.callService('udfordringer','add_points',{ child_id: this._pointsChild.id, points: pts }); this._closePoints(); }
	async _resetPoints(c){ if(!this._pointsEnabled()) return; if(!confirm(this._t('btn.reset_points')+'?')) return; await this.hass.callService('udfordringer','reset_points',{ child_id: c.id }); }
	async _promptRename(c){ const nn=prompt(this._t('btn.rename'), c.name); if(!nn || nn===c.name) return; await this.hass.callService('udfordringer','rename_child',{ child_id:c.id, new_name: nn }); }
	...
	_pickIcon(id){ this._taskIcon=id; try{ const arr=Array.isArray(this._iconRecents)? [...this._iconRecents]:[]; const next=[id,...arr.filter(x=>x!==id)].slice(0,12); this._iconRecents=next; localStorage.setItem('udf_icn_recent', JSON.stringify(next)); }catch{} this._iconModalOpen=false; this.requestUpdate(); }
	...
	_saveCustomIcon(){ const icon = String(this._customIconPreview||'').trim(); const label = String(this._customIconLabel||'').trim(); if(!icon || !label) return; const customs = Array.isArray(this._customIcons)? [...this._customIcons]:[]; const exists = customs.find(c=> c.id === icon); if(!exists){ customs.push({ id: icon, label }); try{ localStorage.setItem('udf_custom_icons', JSON.stringify(customs)); }catch{} this._customIcons = customs; } this._pickIcon(icon); this._closeCustomIconModal(); }
	_deleteCustomIcon(iconId){ const customs = Array.isArray(this._customIcons)? [...this._customIcons]:[]; const filtered = customs.filter(c=> c.id !== iconId); try{ localStorage.setItem('udf_custom_icons', JSON.stringify(filtered)); }catch{} this._customIcons = filtered; this.requestUpdate(); }
	...
	async _createTask(){
		...
		await this.hass.callService('udfordringer','add_task', taskData);
		...
		if (assignToday && _autoAssignIds.length){
			...
			if (tpl?.id){
				for (const cid of _autoAssignIds){
					if(!cid) continue;
					await this.hass.callService('udfordringer','assign_task', { task_id: tpl.id, child_id: cid });
				}
			}
		}
		...
		try{
			if (this._collapsed?.tasks){
				...
				localStorage.setItem('udf_admin_collapsed', JSON.stringify(next));
			}
		}catch{}
	...
	async _setStatus(taskId,status){
		try{
			await this.hass.callService('udfordringer','set_task_status',{ task_id: taskId, status });
		} finally {
			...
			for (const childId of targetIds) {
				try{
					...
					if(template?.id){
						...
						const mode = String(task?.schedule_mode||'').toLowerCase();
						await this.hass.callService('udfordringer','add_task',{
							title: task.title,
	...
	async _assignTask(task, childId){ if(!childId){ alert(this._t('alert.choose_child_first')); return; } await this.hass.callService('udfordringer','assign_task',{ task_id: task.id, child_id: childId }); this._setBonusVisualState(task.id, 'assigned'); task._assignTo=''; this.requestUpdate(); }
	async _assignTaskMulti(task, childIds){
		...
		for (const id of ids){
			try{ await this.hass.callService('udfordringer','assign_task',{ task_id: task.id, child_id: id }); this._setBonusVisualState(task.id, 'assigned'); }
			...
			try{
				...
				for (const cid of childIds){
					...
					if (chosen && (!Array.isArray(chosen.categories) || chosen.categories.length===0)){
						this.hass.callService('udfordringer','update_task',{ task_id: chosen.id, categories: cats });
					}
				}
			}catch{}
		}, 500);
	...
	async _addCategory(){
		...
		await this.hass.callService('udfordringer','add_category', payload);
		this._newCategoryName='';
		this._newCategoryColor='';
	}
	async _setCategoryColor(cat, e){
		try{
			...
			await this.hass.callService('udfordringer','set_category_color',{ category_id: cat.id, color });
		}catch(err){
			console.error('Failed to set category color:', err);
		}
	}
	async _promptRenameCategory(cat){ const nn=prompt(this._t('btn.rename'), cat.name); if(!nn || nn===cat.name) return; await this.hass.callService('udfordringer','rename_category',{ category_id: cat.id, new_name: nn }); }
	async _deleteCategory(cat){ if(!confirm(this._t('btn.delete')+'?')) return; await this.hass.callService('udfordringer','delete_category',{ category_id: cat.id }); }
	...
	async _saveEditedTask(){
		...
		try{
			...
			const repeatOn = (scheduleMode === 'repeat');
			const dueVal = (bonusOn && (scheduleMode==='repeat' || scheduleMode==='weekly' || scheduleMode==='monthly')) ? undefined : (String(this._taskDue||'').trim() || undefined);
			await this.hass.callService('udfordringer','update_task',{
			...
			const _days = repeatOn ? Array.from(this._repeatDays||[]) : [];
			await this.hass.callService('udfordringer','set_task_repeat',{
				task_id: this._editingTask.id,
				repeat_days: _days,
	...
	async _deleteTask(taskId){
		...
		await this.hass.callService('udfordringer','delete_task',{ task_id: taskId });
		this._clearCompleted(taskId);
		this._setBonusVisualState(taskId, 'assigned');
	}
	async _addShopItem(){ if(!this._pointsEnabled()) return; const title=String(this._shopTitle||'').trim(); const price=Number(this._shopPrice||0); if(!title || !Number.isFinite(price)) return; await this.hass.callService('udfordringer','add_shop_item',{ title, price, image: this._shopImage||undefined }); this._shopTitle=this._shopPrice=this._shopImage=''; try{ const el=this.shadowRoot.getElementById('c4k-shop-file'); if(el) el.value=''; }catch{} }
	async _deleteShopItem(id){ if(!this._pointsEnabled()) return; if(!id) return; await this.hass.callService('udfordringer','delete_shop_item',{ item_id: id }); }
	async _clearShopHistory(){
		if(!this._pointsEnabled()) return;
		if(!confirm(this._t('confirm.clear_history'))) return;
		try{
			await this.hass.callService('udfordringer','clear_shop_history',{});
		}catch(err){
			try{
				await this.hass.callService('udfordringer','reset_shop_history',{});
			}catch(err2){
				alert('Service udfordringer.clear_shop_history not available');
			}
		}
	}
	async _onPickImage(e){ if(!this._pointsEnabled()) return; const f=e.target?.files?.[0]; if(!f) return; const reader=new FileReader(); reader.onload= async ()=>{ try{ const dataUrl=reader.result; const ext=(f.name.split('.').pop()||'jpg').toLowerCase(); const name=`udf_${Date.now()}_${Math.random().toString(36).slice(2)}.${ext}`; await this.hass.callService('udfordringer','upload_shop_image',{ filename:name, data:String(dataUrl) }); this._shopImage=`/local/udfordringer/${name}`; } finally { this.requestUpdate(); } }; reader.readAsDataURL(f); }
	...
	async _onPickEditImage(e){ if(!this._pointsEnabled()) return; const f=e.target?.files?.[0]; if(!f) return; const reader=new FileReader(); reader.onload= async ()=>{ const dataUrl=reader.result; const ext=(f.name.split('.').pop()||'jpg').toLowerCase(); const name=`udf_${Date.now()}_${Math.random().toString(36).slice(2)}.${ext}`; await this.hass.callService('udfordringer','upload_shop_image',{ filename:name, data:String(dataUrl) }); this._editItem={ ...this._editItem, image:`/local/udfordringer/${name}` }; this.requestUpdate(); }; reader.readAsDataURL(f); }
	async _saveEditItem(){ if(!this._pointsEnabled()) return; const it=this._editItem; if(!it) return; await this.hass.callService('udfordringer','update_shop_item',{ item_id: it.id, title:String(it.title||'').trim(), price:Number(it.price||0), image: it.image||'', active: !!it.active }); this._editItem=null; this.requestUpdate(); }
	async _toggleItemActive(i,e){ if(!this._pointsEnabled()) return; const active=!!e.target.checked; await this.hass.callService('udfordringer','update_shop_item',{ item_id: i.id, active }); }
	_openAdvanced(i){ if(!this._pointsEnabled()) return; try{ const shop=Object.values(this.hass.states).find(s=> s?.entity_id?.includes('udfordringer_shop')); const latest=shop?.attributes?.items?.find?.((x)=> x.id===i.id); this._advItem=latest||i; }catch{ this._advItem=i; } const steps=Array.isArray(this._advItem.actions)? JSON.parse(JSON.stringify(this._advItem.actions)) : []; this._advSteps=steps.map(st=>{ const t=String(st?.type||'').toLowerCase(); if(t==='service'){ const ent=st.entity_id || st.data?.entity_id || ''; const op=st.service || st.op; return { type:'entity_service', entity_id: ent, op }; } if(t==='entity_service'){ return { type:'entity_service', entity_id: st.entity_id||'', op: st.op||st.service }; } if(t==='delay'){ return { type:'delay', seconds: Number(st.seconds||st.secs||0) }; } return st; }); this.requestUpdate(); }
	...
	async _saveAdvanced(){ if(!this._pointsEnabled()) return; if(!this._advItem) return; const actions=(this._advSteps||[]).map(st=> st.type==='delay'? { type:'delay', seconds: Number(st.seconds||0) } : { type:'entity_service', entity_id: st.entity_id, op: st.op||'turn_on' }); await this.hass.callService('udfordringer','update_shop_item',{ item_id: this._advItem.id, actions }); this._advItem=null; this._advSteps=[]; this.requestUpdate(); }
	async _completeNow(task){
		...
		try{
			...
			this.requestUpdate();
			await this.hass.callService('udfordringer','set_task_status',{ task_id: task.id, status: next, completed_ts: ts });
		} finally {
			this._setTaskBusy(task.id, false);
		}
	}
	async _advance(task){ 
		...
		try{
			...
			if(next === 'awaiting_approval') {
				...
				await this.hass.callService('udfordringer','set_task_status',{ task_id: task.id, status: next }); 
			}
		} finally {
			this._setTaskBusy(task.id, false);
		}
	...
	async _playCompletionSound(){
		try{
			...
			candidates.push('/local/udfordringer/completion.mp3','/local/udfordringer/completion.wav','/local/udfordringer/completion.ogg');
	...
	async _buy(item, childId){
		...
		try{
			await this.hass.callService('udfordringer','buy_shop_item',{ child_id: childId, item_id: item.id });
			purchaseOk = true;
	...
	async _debugMarkOverdue(task) {
		...
		try {
			...
			await this.hass.callService('udfordringer','debug_mark_overdue',{ task_id: task.id });
		} catch(e) {
...
try{ customElements.define('udfordringer-card', UdfordringerDevCard); }catch(e){ /* ignore */ }
// Simple GUI editor
class UdfordringerDevCardEditor extends LitElement{
	...
	async _saveGlobalEnablePoints(enabled){
		try{
			if (!this.hass) return;
			await this.hass.callService('udfordringer','set_ui_colors',{ enable_points: !!enabled });
		}catch(e){
	...
	_saveGlobalColors(cfg){
		...
		try{
			...
			this._uiColorSaveTimer = setTimeout(async ()=>{
				try{
					await this.hass.callService('udfordringer','set_ui_colors', this._pendingUiColors || payload);
				}catch(e){
	...
	async _onSoundUpload(e) {
		...
		reader.onload = async () => {
			try {
				...
				await this.hass.callService('udfordringer','upload_shop_image',{ filename: name, data: String(dataUrl) });
				// Update config with the path
				this._config = {...this._config, completion_sound: `/local/udfordringer/${name}`};
	...
	_getCompletionSoundFilename(){
		try{
			...
			if (!raw) return '';
			const m = raw.match(/\/local\/udfordringer\/([^/?#]+)$/);
			if (m && m[1]) return m[1];
	...
	async _ensureDetectedCompletionSound(force=false){
		try{
			...
			for (const ext of exts){
				const url = `/local/udfordringer/completion.${ext}`;
	...
	async _deleteCompletionSound(){
		try{
			...
			if (filename){
				try{ await this.hass.callService('udfordringer','delete_uploaded_file',{ filename }); }catch(e){ /* fall through */ }
			}
			// Always also delete legacy completion files
			await this.hass.callService('udfordringer','delete_completion_sound',{});
	...
	_saveGlobalUiSettings(payload){
		...
		try{
			...
			if (hasNotifySettings){
				try{ this.hass.callService('udfordringer','set_ui_colors', this._pendingUiColors || next); }catch(e){
			...
			this._uiColorSaveTimer = setTimeout(async ()=>{
				try{
					await this.hass.callService('udfordringer','set_ui_colors', this._pendingUiColors || next);
				}catch(e){
...
try{ customElements.define('udfordringer-card-editor', UdfordringerDevCardEditor); }catch(e){ /* ignore */ }

// Lovelace card registry
window.customCards = window.customCards || [];
window.customCards.push({ type: 'udfordringer-card', name: 'Udfordringer (Parent/Child/Latest)', preview: true, description: 'Combined card – choose Parent, Child, or Latest tasks in the editor' });
