# Udfordringer — starterfork

> Starteren er automatisk afledt af det uploadede projekt og omdøbt fra `chores4kids` til `udfordringer`. Kernen er gjort klar som udgangspunkt, men branding, tekster og eventuelle funktionelle justeringer bør stadig gennemgås manuelt før release.

[![HACS](https://img.shields.io/badge/HACS-Default-blue.svg)](https://hacs.xyz)
[![Downloads](https://img.shields.io/github/downloads/qlerup/chores4kids/total)](https://github.com/YOUR_GITHUB/udfordringer/releases)
...
- 🖼️ **Media upload**: shop images uploaded to `/local/udfordringer/`
...
- Exposes services in the `udfordringer` domain (called by the card)
...
[![Open this repository in HACS](https://my.home-assistant.io/badges/hacs_repository.svg)](https://my.home-assistant.io/redirect/hacs_repository/?owner=YOUR_GITHUB&repository=udfordringer)

1. In HACS → **Integrations** → **⋯ → Custom repositories**
2. Add this repository URL as **Integration**:
   ```
   https://github.com/YOUR_GITHUB/udfordringer
   ```
3. Install **Udfordringer**
4. **Restart** Home Assistant
5. Go to **Settings → Devices & Services → Add Integration → Udfordringer** and press **Submit**
...
2. Add the resource that HACS installed (typically under `/hacsfiles/...`), e.g. something like:
   - `/hacsfiles/udfordringer/udfordringer-card.js`
...
- **Name:** `Udfordringer Points {Child Name}`
...
- **Entity:** `sensor.udfordringer_tasks`
- **State:** number of tasks
- **Attributes:** `tasks` (full list)

#### 3) Shop (optional)
- **Entity:** `sensor.udfordringer_shop`
...
- Save the plan — the card calls `udfordringer.set_task_repeat`
...
## Services (domain: `udfordringer`) 🛠️

### Children admin
- `udfordringer.add_child`
- `udfordringer.rename_child`
- `udfordringer.remove_child`
- `udfordringer.add_points`
- `udfordringer.reset_points`

### Tasks
- `udfordringer.add_task`
- `udfordringer.assign_task`
- `udfordringer.set_task_status`
- `udfordringer.approve_task`
- `udfordringer.delete_task`
- `udfordringer.set_task_repeat`
- `udfordringer.set_task_icon`

### Shop
- `udfordringer.add_shop_item`
- `udfordringer.update_shop_item`
- `udfordringer.delete_shop_item`
- `udfordringer.buy_shop_item`
- `udfordringer.upload_shop_image` (saves to `/config/www/udfordringer/` for `/local/udfordringer/<file>`)

### Maintenance
- `udfordringer.purge_orphans` — remove leftovers from older versions
...
- **No children listed**
  - Create children in Admin view (or call `udfordringer.add_child`)
- **No tasks**
  - Create one in Admin view (or call `udfordringer.add_task`)
- **Shop empty for kids**
  - Ensure items are **Active**
- **Shop images not showing**
  - Use `udfordringer.upload_shop_image`, then reference `/local/udfordringer/<filename>`
- **Leftover sensors/devices after upgrade**
  - Run `udfordringer.purge_orphans`
