...
async def _ensure_static_frontend_path(hass: HomeAssistant) -> None:
    """Expose frontend files from custom_components/udfordringer/frontend at /udfordringer."""
    hass.data.setdefault(DOMAIN, {})

    if hass.data[DOMAIN].get("frontend_static_registered"):
        return

    frontend_path = Path(__file__).parent / "frontend"

    if not frontend_path.exists():
        _LOGGER.warning("%s: frontend folder not found: %s", DOMAIN, frontend_path)
        return
    ...
    _LOGGER.debug("%s: registered static frontend path at /udfordringer -> %s", DOMAIN, frontend_path)


async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    """Set up the Udfordringer integration."""
    ...
    # Expose frontend files like:
    # /udfordringer/udfordringer-card.js?v=0
    try:
        await _ensure_static_frontend_path(hass)
    except Exception:
        _LOGGER.debug("%s: static frontend registration failed", DOMAIN, exc_info=True)
    ...
    def _resolve_notify_image_url(raw: str) -> str:
        try:
            ...
            if url.startswith("http://") or url.startswith("https://"):
                return url
            base = str(getattr(hass.config, "external_url", "") or "").strip() or str(
        ...
        try:
            ...
            if bool(getattr(task, "bonus_enabled", False)):
                ...
                bonus_status = str(
                    texts.get(
                        bonus_status_key,
                        "Bonus task completed" if bonus_status_key == "bonus_done" else "Bonus task not completed",
                    )
                )
                bonus_line_tpl = str(texts.get("bonus_line", "{status}"))
                message = f"{message}\n{bonus_line_tpl.format(label=bonus_label, status=bonus_status)}"
            tag = f"udfordringer_task_done_{task_id}"
    ...
    async def _notify_shop_purchase(purchase):
        try:
            ...
            data = {"tag": "udfordringer_shop_purchase"}
        ...
        try:
            ...
            if not task_id:
                try:
                    ...
                    if tag.startswith("udfordringer_task_done_"):
                        task_id = tag.split("udfordringer_task_done_", 1)[1].strip()
                except Exception:
                    pass
    ...
    async def svc_upload_shop_image(call: ServiceCall):
        ...
        rel_dir = hass.config.path("www", "udfordringer")
        ...
        try:
            ...
            raise ValueError("invalid_base64") from ex
    ...
    async def svc_delete_uploaded_file(call: ServiceCall):
        """Delete a previously uploaded file from /config/www/udfordringer."""
        import os
        import re
    ...
    async def svc_delete_completion_sound(call: ServiceCall):
        """Delete completion sound files from /config/www/udfordringer."""
        import os
        import re
    ...
    async def svc_purge_orphans(call: ServiceCall):
        ...
        for e in reg_entries:
            if e.platform != Platform.SENSOR:
                continue
            uid = e.unique_id or ""
            if uid.startswith("udfordringer_points_"):
                suffix = uid.replace("udfordringer_points_", "")
                ...
                if desired is None:
                    desired = dev_registry.async_get_or_create(
                        ...
                        manufacturer="Udfordringer",
                        model="Virtual Child",
                        name=f"Udfordringer – {suffix}",
                    )
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    if unload_ok:
        ...
        unsub = domain_data.pop("notify_action_unsub", None)
        if unsub:
            try:
                unsub()
            except Exception:
                _LOGGER.debug("%s: notify action unsubscribe failed", DOMAIN, exc_info=True)
        unsub_ios = domain_data.pop("notify_action_unsub_ios", None)
        if unsub_ios:
            try:
                unsub_ios()
            except Exception:
